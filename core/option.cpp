...
Option::Option(const char *l, const char *d) { init(l, d); }
...
#define add_option(T)                                                                              \
    desc(option,                                                                                   \
         po::value<T>(any_cast<T *>(target_variable))->default_value(any_cast<T>(default_value)),  \
         description)
    ...
    if (default_value.type() == typeid(int)) {
        ...
        desc(option,
             po::bool_switch(any_cast<bool *>(target_variable))
                 ->default_value(any_cast<bool>(default_value)),
             description);
    } else if (default_value.type() == typeid(double)) {
        add_option(double);
    } else if (default_value.type() == typeid(string)) {
