...
template <typename T> struct atomic_wrapper {
    ...
    atomic_wrapper(const std::atomic<T> &a) : _a(a.load(memory_order_relaxed)) {}

    atomic_wrapper(const atomic_wrapper &other) : _a(other._a.load(memory_order_relaxed)) {}
    ...
    T operator()() const { return _a.load(memory_order_relaxed); }

    T load() const { return _a.load(memory_order_relaxed); }
