...
settings_parser::settings_parser(int argc, char **argv) {
    ...
    options.push_back(Option("threads,t", "set the number of parallel threads",
                             std::thread::hardware_concurrency(), &s.threads));
    ...
    options.push_back(Option("out,o", "output filename (defaults to the loaded checkpoint stem)",
                             "output", &s.outfile));
    options.push_back(
        Option("no-image", "save the checkpoint but skip TIFF output", false, &s.no_image));
    options.push_back(Option("load,L", "try to load a previously saved state", "", &s.infile));
    options.push_back(Option("allow-legacy-checkpoint",
    ...
    options.push_back(
        Option("formula", "specify the formula to evaluate", "z = z * z + c", &s.formula));
    ...
    if (s.infile != "" && vm["out"].defaulted()) {
        ...
        if (s.outfile.size() >= zstd_suffix.size() &&
            s.outfile.compare(s.outfile.size() - zstd_suffix.size(), zstd_suffix.size(),
                              zstd_suffix) == 0)
...
settings_parser::settings_parser(const string &filename) { load(filename); }

settings settings_parser::operator()() { return s; }
